...
The local Smithers workflow workspace is validated separately from the Zig release gates:
...
The expected workflow-list smoke output is:
...
The Kanban workflow runs every markdown ticket under `.smithers/tickets/` in worktree branches. Invoke it only when the maintainer wants the workflow to begin ticket execution:

```bash
cd .smithers
bun run workflow:run -- kanban --input '{"maxConcurrency":3}' --detach --log-dir ./.logs/smithers
```

Detached runs print a Smithers run identifier and write the NDJSON event log under `./.logs/smithers/`. Use that run identifier with `bun run workflow:run -- kanban --resume <run-id>` if the run needs to be resumed.
...
This is the required CI path for release-candidate confidence. It currently verifies the active official suite coverage recorded in `.smithers/external-test-suites.json`; it is not a claim that every Hive simulator or every Ethereum fixture directory is already fully active.

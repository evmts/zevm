...
## Canonical Local Generation + Validation

Use these repository scripts for non-interactive candidate checks:

```bash
./scripts/release-metadata/generate.sh --release-identifier vX.Y.Z --out-dir .ops/release-metadata/vX.Y.Z
./scripts/release-metadata/validate.sh --release-identifier vX.Y.Z --in-dir .ops/release-metadata/vX.Y.Z
```

`validate.sh` fails fast when either required artifact is missing or when file schema/version/identifier/value invariants do not match the candidate tuple for that release input.

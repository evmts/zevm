...
Use canonical scope sources instead of repeating the full checklist:
...
- deferred-surface promotion and boundary/graduation criteria: [Specs And Process](../specs-and-process#deferred-surface-promotion-and-phase-1-boundary-and-graduation-criteria)

The current contract remains source-first; release artifact jobs may publish CLI/C ABI/npm artifacts, while installer UX, signing/notarization, and byte-identical binary reproducibility guarantees are out of scope.
...
Phase-1 transport limits are fixed, not configurable: `64` active TCP connections, `15,000` ms read/write socket timeouts, `8,192` byte HTTP header buffer, and `8,388,608` byte request body limit. Larger request bodies return HTTP `413` without a JSON-RPC body.

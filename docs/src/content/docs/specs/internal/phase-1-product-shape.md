...
- phase-1 installation contract is source-first with package-manager dependency pins
- canonical build commands are `zig build --fetch` and `zig build`
...
- phase-1 source-build provenance has two states only: metadata-backed published-release provenance or operator-recorded unreleased-commit provenance
- metadata-backed operator flow is strict: select one `releaseIdentifier`, fetch/validate required assets for that identifier, materialize tuple-pinned ZEVM/toolchain/package pins, then build
- unreleased-commit flow has no release-asset discovery; operators self-record tuple pins from local state and must not claim metadata-backed reproducibility
- operator verification path is: materialize the pinned ZEVM commit, verify dependency URL pins with `zig build dependency-preflight`, and verify `zig version` equals the tuple `zigVersion`
...
- installer UX, signing/notarization, byte-identical reproducible-build hashes, and one-host cross-compilation guarantees are explicitly out of scope in phase 1

## 4. Release Qualification And Verification (PRD 3.5 Summary)

- clean-checkout qualification requires `zig build --fetch`, `zig build`, and `zig build test` success for pinned ZEVM/package commits without manual sibling repair or ad-hoc dependency edits
...
- assertion mapping records are structured qualification entries and include, at minimum: `surfaceId`, `surfaceSection`, `surfaceCategory` (`startup`, `configuration`, `runtime`, `transport`, `method`, `release-asset`), `assertionType`, `assertionIdentifier`, and expected contract outcome
...
- qualification coverage is actionable only when mapping rows explicitly cover startup/configuration semantics (PRD section 5), runtime/lifecycle semantics (sections 4, 8, 9, 10, 11, 12), transport semantics (section 6), method tuple/error semantics (section 7 + JSON-RPC contract), and release-asset semantics (sections 3.3 and 3.4)

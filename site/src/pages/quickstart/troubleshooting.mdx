...
Before troubleshooting startup/runtime behavior, confirm [Installation](/quickstart/installation) prerequisites are satisfied (Zig version, Rust/Cargo, package-manager dependency fetch, and successful source build output at `./zig-out/bin/zevm`).
...
Runtime JSON-RPC request telemetry is emitted on process `stderr` as JSON log records with `scope = "rpc"`.
...
| `413` | Request body is larger than `8,388,608` bytes | Split the request, reduce batch size, or move oversized tooling payloads outside the JSON-RPC call |
...
# 413: body larger than the fixed 8 MiB transport limit

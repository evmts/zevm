...
If you only need direct contract-source links, use [Canonical Specs](/reference/canonical-specs).
...
- public pages that inventory deferred surfaces (for example [Unsupported And Deferred](/reference/json-rpc/unsupported-and-deferred)) are summaries and do not independently promote contract status
...
For end-to-end procedure (fetch, validation, pinned checkouts, manifest provenance, runtime baked-default audit), use [Release Metadata Runbook](/reference/release-metadata-runbook).
...
- CI runs the root release gates documented in [CI And Release Gates](/reference/ci-and-release-gates), including package dependency preflight, strict qualification coverage, generated-metadata smoke validation, dirty-worktree preflight, and scheduled/manual external verification
...
- [CI And Release Gates](/reference/ci-and-release-gates)

...
- Contract ownership and docs-first change flow: [Canonical Specs And Docs-First Process](/reference/specs-and-process)
...
- baked default checkpoint values are release/build-specific startup data; this page defines runtime/startup error semantics, while startup knobs and precedence are specified in [Light-Mode Configuration](/reference/configuration/light-mode) and [Configuration Overview](/reference/configuration/overview)
...
- Core reads: `eth_chainId`, `eth_blockNumber`, `eth_getBalance`, `eth_getCode`, `eth_getStorageAt`, `eth_getStorageValues`, `eth_getProof`, `eth_getTransactionCount`, `eth_accounts`, `eth_coinbase`, `eth_gasPrice`, `eth_maxPriorityFeePerGas`, `eth_blobBaseFee`, `eth_feeHistory`
...
- Compatibility utilities: `rpc_modules`, `web3_clientVersion`, `web3_sha3`, `net_version`, `net_listening`, `net_peerCount`, `eth_mining`, `eth_hashrate`, `eth_getWork`, `eth_submitWork`, `eth_submitHashrate`, `eth_syncing`, `eth_protocolVersion`
...
- simulation and execution-testing methods are trusted-only in phase 1; light-mode `eth_call`, `eth_estimateGas`, `eth_createAccessList`, `eth_simulateV1`, and `testing_buildBlockV1` return `-32010`, and `eth_call` remains a deferred light-mode proof-backed target

> Readiness gate: while light mode is not ready, readiness-gated calls return `-32011`. See [Verified Light-Mode Reads Methods](/reference/json-rpc/verified-light-mode-reads#methods) and [Readiness And Proof Rules](/reference/json-rpc/verified-light-mode-reads#readiness-and-proof-rules).
...
- [Canonical Specs And Docs-First Process](/reference/specs-and-process)

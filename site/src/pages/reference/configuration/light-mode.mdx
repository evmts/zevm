...
For a concrete reverse-proxy config example, see [Configuration Overview -> RPC Exposure And Hardening](/reference/configuration/overview#rpc-exposure-and-hardening).
...
For published release identifiers, release-specific baked defaults are published in `light-default-checkpoints.json`; see [Canonical Specs And Docs-First Process](/reference/specs-and-process#release-metadata-records), [Release Metadata Runbook](/reference/release-metadata-runbook), and [ZEVM release metadata](https://github.com/evmts/zevm/releases).
...
Quick audit snippet (requires `jq` and local release metadata files prepared per [Release Metadata Runbook](/reference/release-metadata-runbook)):
...
Readiness semantics and selector contracts are defined in [Verified Light-Mode Reads](/reference/json-rpc/verified-light-mode-reads#readiness-and-proof-rules).

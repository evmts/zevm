...
This page summarizes ZEVM HTTP JSON-RPC 2.0 behavior shared across trusted mode and light mode.
...
- light mode always-callable `zevm_lightSyncStatus` and `eth_chainId`, plus readiness-gated `eth_blockNumber` and proof-backed reads
- phase 1 installation/distribution guarantee is source-build installation only
- packaged binaries, installers, and distribution-channel guarantees are out of scope for phase 1
...
- For transport-level debugging/remediation of `404`/`405`/`415`/`204`, see [Troubleshooting -> HTTP Transport Statuses (JSON-RPC)](../../quickstart/troubleshooting#http-transport-statuses-json-rpc)
...
- Docs-first change flow and release/versioning policy: [Canonical Specs And Docs-First Process](../specs-and-process)
- When auditing a pinned build or published release, read `docs/specs/prd.md` and `docs/specs/json-rpc-contract.md` from that exact ZEVM checkout rather than repository `main`
...
- well-formed requests for supported methods or selectors unavailable in active mode return `-32010`
...
Light numeric-selector note: while `ready = false`, readiness-gated calls fail with `-32011` before retained-history numeric-range validation; once `ready = true`, numeric selectors outside the retained set fail with `-32602`.

## Method Surface By Mode

### Trusted mode

Family-level summary only. Canonical method inventories and alias sets live on the linked method pages to avoid drift.
...
- Queries: `eth_getBlockByNumber`, `eth_getBlockByHash`, `eth_getBlockTransactionCountByNumber`, `eth_getBlockTransactionCountByHash`, `eth_getTransactionByHash`, `eth_getTransactionByBlockNumberAndIndex`, `eth_getTransactionByBlockHashAndIndex`, `eth_getTransactionReceipt`, `eth_getBlockReceipts`, `eth_getLogs`
...
Canonical light-mode method tuples and selector/error order are maintained on [Verified Light-Mode Reads](./verified-light-mode-reads) to avoid drift.

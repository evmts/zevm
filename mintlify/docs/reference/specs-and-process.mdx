...
Use [Canonical Specs](./canonical-specs) as the stable public entry point. The source-file links in this section point at repository `main`; for release-specific or pinned-build audits, read the same files from the exact ZEVM checkout selected through [Release Metadata Runbook](./release-metadata-runbook).
...
3. Update public Mintlify pages under `mintlify/docs/`.
4. In that same docs cycle, reconcile the active traceability map and Mintlify maintenance plan so page ownership, anchors, and verification dates stay aligned with the updated public pages.

Planning/history files under `docs/plans/` and `docs/context/` are non-normative unless explicitly marked otherwise.
...
- deferred/out-of-contract status changes only through docs-first updates to the canonical spec sources listed on [Canonical Specs](./canonical-specs)
- update order is mandatory: canonical spec sources first, then internal support docs, then public Mintlify docs
- the same docs cycle must also reconcile page-level traceability metadata so the public pages and maintenance map do not drift
...
- phase-1 boundary or graduation claims are canonical only after matching updates land in the same docs-first path above; planning/context files are non-normative
...
- clean-checkout source build verification passes: `zig build` and `zig build test` succeed on pinned siblings/toolchain without local patching
...
- for candidates claiming a published `releaseIdentifier`, qualification validates both required release artifacts, all PRD section `3.4` field/schema/value invariants, and candidate build/runtime equality against the asset-declared tuple/default values
- qualification evidence for metadata-backed candidates includes release-provenance and supersession-note assertions when PRD section `3.4` operator-flow/correction-note invariants apply

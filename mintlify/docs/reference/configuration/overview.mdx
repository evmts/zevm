...
- phase-1 operator-facing startup inputs are `network` and `consensusRpcUrl`, with optional checkpoint controls (`checkpoint`, `checkpointDir`, `maxCheckpointAgeSeconds`, `strictCheckpointAge`); proof-source internals are not user-configurable in the public contract
...
- `status = "error"` keeps `ready = false`; if persistent, operators should verify `network`, `consensusRpcUrl`, and checkpoint inputs, then restart

...
Operator runbook cross-link: [Run Light Mode -> Choose Startup Checkpoint Input](../../quickstart/run-light-mode#2-choose-startup-checkpoint-input) and [Run Light Mode -> Check Readiness With `zevm_lightSyncStatus`](../../quickstart/run-light-mode#check-readiness-with-zevm-light-sync-status).
...
If the selected build boundary has no valid published `light-default-checkpoints.json`, do not treat default selection as metadata-backed; continue only under operator-recorded provenance and provide an explicit checkpoint when deterministic startup selection is required.
...
4. confirm `checkpointSource = "default"`; if sampled immediately after startup, `lastCheckpoint` can be compared to the selected release/build's `light-default-checkpoints.json` value for that network as an early-state validation only (it may advance during sync)
...
Expect `checkpointSource` to be `persisted`. If sampled immediately after startup, `lastCheckpoint` may briefly equal `0x` plus your file value (case-insensitive hex), but it can advance during sync and should not be treated as a stable invariant.

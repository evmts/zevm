...
function printLines(value, maxLines) {
    const lines = value.trimEnd().split("\n").filter(Boolean);
    for (const line of lines.slice(0, maxLines))
        process.stderr.write(`${line}\n`);
    if (lines.length > maxLines)
        process.stderr.write(`... ${lines.length - maxLines} more line(s)\n`);
}
async function checkDependencyManifest() {
...
function checkCleanGit(name, path) {
...
async function checkQualificationMap() {
    try {
...
async function checkExternalSuiteEvidence() {
...
function checkLaunchPreflight() {
    if (!includeLaunchPreflight)
        return;
    const result = run(["bun", "tools/macos_launch_policy_preflight.ts"]);
    process.stdout.write(result.stdout);
    process.stderr.write(result.stderr);
    if (result.exitCode !== 0)
        fail("local executable launch preflight failed");
}

...
writeFileSync(probeSource, `#include <stdio.h>
...
if (timedOut) {
    process.stderr.write(`error: newly built local executables did not reach main within ${timeoutMillis / 1000}s on this macOS host\n`);
    ...
    if (Bun.which("spctl"))
        printResult(["spctl", "--assess", "-vv", probeBinary]);
    if (Bun.which("xattr"))
        printResult(["xattr", "-l", probeBinary]);
    cleanup();
    process.exit(1);
}
if (code !== 0) {
